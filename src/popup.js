browser.storage.local.get('selectedText').then(result => {
    ...
    inputUa.value = text;
    const isFirstLetterUkrainian = checkIfFirstLetterUkrainian(text);

    if (isFirstLetterUkrainian) {
        const translatedTextEn = translateToEnglish(text);
...
// Перевірка розкладки клавіатури
